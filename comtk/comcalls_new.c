...
out_message  hc11_cmd_md(int fd,char* command)
{
  char buffer[10000];  /* Input buffer */
  char out_buff[10000];
...
out_message  hc11_cmd_mm(int fd,char *memory_address,char *memory_tag)
{
  char buffer[10000];  /* Input buffer */
  char out_buff[10000];
...
out_message  send_string(int fd,char *inp_str)
{
  ...
  sprintf(ascii_length,"%d",inp_str_len);
  ...
  strcat(buffer,"e_x\n");/*this is the sieg_proc*/
  write(fd,text_begin,strlen(text_begin));
  write(fd,preamble,strlen(preamble));
